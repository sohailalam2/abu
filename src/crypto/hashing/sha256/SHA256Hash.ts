...
import {
  ...
  HashFunction,
  HashFunctionConfig,
} from '@/crypto/hashing/HashFunction';
...
export interface SHA256HashConfig extends HashFunctionConfig {
  encoding: 'hex' | 'bin';
}

export class SHA256Hash implements HashFunction<string, string | Uint8Array> {
  ...
  hash(payload: string, config?: SHA256HashConfig): string | Uint8Array {
    if (!hasValue(payload)) {
      throw new InvalidPayloadProvidedToHashFunctionException(this.constructor.name);
    }

    return new SHA256Hash().update(payload).digest(config?.encoding || 'hex');
  }

  private update(data: string, encoding?: string): this;
  private update(data: Uint8Array): this;
  private update(data: ArrayBufferView): this;

  private update(data: string | Uint8Array | ArrayBufferView): this {
    // data: string
  ...
  private digest(encoding: string): string;
  private digest(encoding?: string) {
    const { _byte, _word } = this;
